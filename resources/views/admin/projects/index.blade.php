    ...
    <div class="box-header with-border">
        ...
        <div class="box-action pull-right">
            <ul class="header-action">
                <li><a href="{{url('admin/projects/create')}}" class="btn btn-primary"><i class="fa fa-plus-square"></i> {{__('Thêm mới')}}</a></li>
                <li><a href="{{url('admin/projects')}}" class="btn btn-default" ><i class="fa fa-refresh"></i> {{__('Tải lại')}}</a></li>
            </ul>
        </div>
    </div>
    <div class="box-body">
        <div class="box-body table-responsive no-padding">
            <div class="box-search-table pull-right">
                <div class="box-delete_multi pull-left">
                    <a href="javascript:;" id="btn-delete-all" data-routes="/admin/projects/destroy" class="btn btn-danger"><i class="fa fa-trash-o"></i> Xóa tất cả</a>
                </div>
                {!! Form::open(array('url' => url("admin/projects"), 'id' => 'form-search', 'method' => 'GET')) !!}
            ...
            <table id="table-main" class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center "><input type="checkbox" class="checkItem" value="{{$project->id}}" /></td>
                        <td class="text-center">{{$no++}}</td>
                        <td>{{$project->name}}</td>
                        <td class="text-center w_100">
                            <input type="checkbox" value="{{$project->id}}" data-size="mini" data-on-text="Hiện" data-off-text="Ẩn" data-on-color="success" data-off-color="danger" name="is_visible" {{$project->is_visible?'checked':''}} />
                        </td>
                        <td class="action">
                            <a href="{{url('admin/projects/edit/' . $project->id)}}" class="btn btn-primary" title="{{trans('common.edit')}}"><i class="fa fa-edit"></i></a>
                            <a href="javascript:;" onclick="deleteModal('{{$project->id}}', '/admin/projects/destroy')" title="{{trans('common.delete')}}" class="btn btn-danger"><i class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
                    @endforeach
                    @else 
            ...
            <div class="footer-table">
                <div class="col-sm-6">
                    <span class="total-record">{!!__("Tổng cộng: <strong>:total</strong> kết quả", ['total' => $projects->total()])!!}</span>
                </div>
                <div class="col-sm-6">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('input[name="is_visible"]').on('switchChange.bootstrapSwitch', function (event, state) {
            ...
            $.ajax({
                url: root + '/admin/projects/active', 
                type: 'POST',
